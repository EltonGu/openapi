...
###3.8.17 <span id="3-8-17">获取运单状态</span>
```
MDPLocationCollectionManager.getShipmentStatus(Context context, String shipmentCode, OnResultListener listener)
```
...
返回值
* 成功时回调onSuccess(Object result), result model为ShipmentStatusEnum, model 详见附录。
* 失败时回调onFailure(String errorCode, String errorMessage);
* 可能发生的Error Code：（参见本文档最后的附录）OB010503, SS000000, SS000001
