...
###3.9.1 <span id="3-9-1">验证企业应用和司机身份证号码</span>
```
...
###3.9.2 <span id="3-9-2">检查NFC匹配结果</span>
```
...
###3.9.7 <span id="3-9-7">运单卸货照片上传</span>
```
...
###3.9.8 <span id="3-9-8">运单回单照片上传</span>
```
...
###3.9.9 <span id="3-9-9">获取运单卸货照片文件名列表</span>
```
...
###3.9.10 <span id="3-9-10">获取运单回单照片文件名列表</span>
```
...
###3.9.11 <span id="3-9-11 ">运单卸货照片删除</span>
```
...
###3.9.12 <span id="3-9-12">运单回单照片删除</span>
```
...
###3.9.13 <span id="3-9-13">下载卸货照片</span>
```
...
###3.9.14 <span id="3-9-14">下载回单照片</span>
```
...
###3.9.15 <span id="3-9-15">获取待同意发票列表</span>
```
支持分页，以发票申请的时间倒序
...
###3.9.16 <span id="3-9-16">同意发票申请</span>
```
...
###3.9.17 <span id="3-9-17">获取运单状态</span>
```
MDPLocationCollectionManager.getShipmentStatus(Context context, String shipmentCode, String enterpriseCode, OnResultListener listener)
```

参数说明
* @param context     上下文
* @param shipmentCode     运单编号
* @param enterpriseCode   企业编号 
* @param listener    回调函数

返回值
* 成功时回调onSuccess(Object result), result model 为ShipmentStatusEnum, 详见附录。
* 失败时回调onFailure(String errorCode, String errorMessage);
* 可能发生的Error Code：OB010503, SS000000, SS000001
